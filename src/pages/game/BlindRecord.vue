...
<template>
  ...
  <van-overlay z-index="2000" :show="show" @click="emit('onClose')" class="bg-op-50!">
    <div class="wrapper-1">
      <div
        class="text-[12px] py-2 h-[calc(100%-70px)] w-[260px] bg-[rgb(29,29,29,0.8)]   flex flex-col pos-relative  border-rd-[15px]!"
        @click.stop>
        ...
        <div class="  h-full w-full flex flex-col justify-start mt-[10px] items-center text-white">
          <div class="w-[90%] flex flex-row justify-between  font-bold ">
            <p>盲注:{{ roomStore.blindRecored?.big_blind }}/{{ roomStore.blindRecored?.limit_blind }}</p>
