...
const buyMoney = ref((roomStore.roomUserInfo.balance * 0.33).toFixed(2))
const addBet = () => {
    ...
    <div class="wrapper2 pos-relative">
      <div
        ...
        <!-- <van-slider bar-height="10px" class="w-[95%]! mr-[10px]" active-color="#e1bf65" v-model="buyMoney" :step="5"
          :min="0" :max="roomStore.roomUserInfo.balance">
          <template #button>
       ...
       <div class="flex flex-row justify-evenly flex-wrap">
         <button class="image-button my-2">
          <div @click="buyMoney = (roomStore.roomUserInfo.balance * 0.33).toFixed(2)"
            ...
            <p class="z-10 text-white font-bold text-[15px]">{{ (roomStore.roomUserInfo.balance * 0.33).toFixed(2) }}</p>
          </div>
        </button>
        <button class="image-button">
          <div @click="buyMoney = (roomStore.roomUserInfo.balance * 0.5).toFixed(2)"
            class=" pos-relative flex justify-center items-center w-[100px] aspect-ratio-[14/5]">
            <img class="absolute top-0 left-0 w-full  " src="../../assets/imgae/gamebtn/btn_green.png" />
            <p class="z-10 text-white font-bold text-[15px]">{{ (roomStore.roomUserInfo.balance * 0.5).toFixed(2) }}</p>
          </div>
        </button>
        <button class="image-button my-2">
          <div @click="buyMoney = (roomStore.roomUserInfo.balance * 0.66).toFixed(2)"
            class=" pos-relative flex justify-center items-center w-[100px] aspect-ratio-[14/5]">
            <img class="absolute top-0 left-0 w-full  " src="../../assets/imgae/gamebtn/btn_yellow.png" />
            <p class="z-10 text-white font-bold text-[15px]">{{ (roomStore.roomUserInfo.balance * 0.66).toFixed(2) }}</p>
          </div>
        </button>
